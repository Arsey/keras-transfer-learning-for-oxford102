...
from easyclassify.utils import sizeof_fmt, filesystem as fs
...
class Job(StatusCls):
    ...
    def __init__(self, name, persistent=True):
        ...
        self._id = '{}-{}'.format(time.strftime('%Y%m%d-%H%M%S'), os.urandom(2).encode('hex'))
        self._dir = os.path.join(JOBS_DIR, self._id)
        self._name = name
        self.tasks = []
        self.exception = None
    ...
    def load(cls, job_id):
        ...
        """
        from easyclassify.task.train import TrainTask
    ...
    def save(self):
        ...
        try:
            ...
            print('move tmp pickle to regular path')
