...
import easyclassify
from easyclassify import device_query
from .task import Task
from easyclassify.utils import subclass, override
from easyclassify.dataset import DATASETS_DIR
...
class TrainTask(Task):
    ...
    @override
    def before_run(self):
        self.train_log = open(self.path(self.TRAIN_LOG), 'a')
    ...
    def combined_graph_data(self, cull=True):
        ...
        if self.train_outputs and 'epoch' in self.train_outputs:
            if cull:
                ...
                # return all dataath.dirname(path) == con
                stride = 1
    ...
    def task_arguments(self, resources, env):
        args = [
            sys.executable,
            os.path.join(os.path.dirname(os.path.abspath(easyclassify.__file__)), 'train.py'),
            '--model=%s' % self.network,
            '--data_dir=%s' % os.path.join(DATASETS_DIR, self.dataset),
            '--nb_epoch=%s' % self.train_epochs,
