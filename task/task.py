...
from easyclassify import utils
from easyclassify.job.job import JOBS_DIR
from easyclassify.status import Status, StatusCls
...
class Task(StatusCls):
    ...
    def set_logger(self):
        self.logger = easyclassify.log.JobIdLoggerAdapter(
            logging.getLogger('easyclassify.app'),
            {'job_id': self.job_id},
        )
    ...
    def on_status_update(self):
        ...
        from easyclassify.app import scheduler
    ...
    def path(self, filename, relative=False):
        ...
        if os.path.isabs(filename):
            ...
            if relative:
                path = os.path.relpath(path, JOBS_DIR)
    ...
    def run(self, resources):
        ...
        pprint(args)
        print("***ENV***")
        exclude_env = ['LS_COLORS', 'LIBRARY_ROOTS', 'PYTHONPATH']
        for e in exclude_env:
            if e in env:
                print('env delete', e)
                del env[e]
        pprint(env)
            ...
            while self.p.poll() is None:
                for line in utils.nonblocking_readlines(self.p.stdout):
                    if self.aborted.is_set():
                        print('aborted is set')
                        if sigterm_time is None:
                            print('sigterm time is none')
                    ...
                    if line:
                        ...
                        time.sleep(0.05)
                print('check sigterm time', sigterm_time)
    ...
    def preprocess_output_digits(self, line):
        ...
        if match:
            ...
            timestamp = time.mktime(time.strptime(timestr, easyclassify.log.DATE_FORMAT))
            level = match.group(2)
